...
interface TaskListProps {
  title: string;
  tasks: Task[];
  onUpdateStatus: (id: string, status: TaskStatus) => Promise<void> | void;
...
const TaskList: React.FC<TaskListProps> = ({
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    await onUpdateStatus(taskId, targetStatus); // ✅ backend will place bottom
  };
  ...
  return (
    <div
      className={`flex flex-col w-72 flex-shrink-0 bg-[#f1f2f4] rounded-xl transition-all h-full ${
